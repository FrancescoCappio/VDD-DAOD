...
class resnet(_fasterRCNN):
  def __init__(self, classes, num_layers=101, pretrained=False, class_agnostic=False, lc=False, gc=False):
    ...
    if self.layers == 50:
      self.model_path = 'pretrained/resnet50_caffe.pth'
  ...
  def _init_modules(self):
    ...
    self.netD_ds = netD(context=self.gc)
    self.netD_base = netD(context=self.gc)
    self.RCNN_top_base = nn.Sequential(resnet.layer4)
    self.RCNN_top_di = copy.deepcopy(nn.Sequential(resnet.layer4))
    ...
    self.RCNN_cls_score_base = nn.Linear(feat_d, self.n_classes)
    self.RCNN_cls_score_di = nn.Linear(feat_d, self.n_classes)
    if self.class_agnostic:
      self.RCNN_bbox_pred_base = nn.Linear(feat_d, 4)
      self.RCNN_bbox_pred_di = nn.Linear(feat_d, 4)
    else:
      self.RCNN_bbox_pred_base = nn.Linear(feat_d, 4 * self.n_classes)
      self.RCNN_bbox_pred_di = nn.Linear(feat_d, 4 * self.n_classes)
    ...
    self.RCNN_top_base.apply(set_bn_fix)
    self.RCNN_top_di.apply(set_bn_fix)
  ...
  def train(self, mode=True):
    ...
    if mode:
      ...
      self.RCNN_top_base.apply(set_bn_eval)
      self.RCNN_top_di.apply(set_bn_eval)

  def _head_to_tail_base(self, pool5):
    fc7 = self.RCNN_top_base(pool5).mean(3).mean(2)
    return fc7

  def _head_to_tail_di(self, pool5):
    fc7 = self.RCNN_top_di(pool5).mean(3).mean(2)
    return fc7
